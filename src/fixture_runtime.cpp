...
Zkymzl11Profile Zkymzl11Profile::load_from_file(const std::string& path) {
    ...
    profile.left_pan_scale = std::clamp(number_field(calibration, "left_pan_scale", profile.left_pan_scale), 0.0, 1.0);
    profile.target_y_offset = std::clamp(number_field(calibration, "target_y_offset", profile.target_y_offset), -0.5, 0.5);
